        ...
        if IsPedInAnyVehicle(ped, false) then
            ...
            if ((speedBuffer[2] ~= nil and velBuffer[2] ~= nil) and ((speedBuffer[2] > (MinimaVelocidadEjeccion / 3.6) and (speedBuffer[2] - speedBuffer[1]) > (speedBuffer[1] * seatbeltSpeedPedOut)) or (speedBuffer[1] > (MinimaVelocidadEjeccion / 7.2) and (speedBuffer[1] - speedBuffer[2]) > (speedBuffer[2] * seatbeltSpeedPedOut)))) then
                if not cinturon then
                    if not Moto then
                        if not Bici then
                            TriggerEvent('carhud:fueraVehiculo')
                        end
                    end
                else
                end
    ...
    while true do
        if not Bici then
            if fuel < 20 and not reserva then
                reserva = true
                TriggerEvent('LIFE_CL:Sound:PlayWithinDistance', GetPlayerServerId(PlayerId()), 4, 'advertenciareserva', 0.5)
                ...
                if not Barco then
                    if not Heli then
                        if not limitador then
                            ...
                            TriggerEvent('LIFE_CL:Sound:PlayWithinDistance', GetPlayerServerId(PlayerId()), 4, 'limitador', 0.5)
                            limitador = true
                        else
                ...
                if not Barco then
                    if not Heli then
                        if not cinturon then
                            ...
                            TriggerEvent('LIFE_CL:Sound:PlayWithinDistance', GetPlayerServerId(PlayerId()), 4, 'cinturonpuesto', 0.5)
                        else
                            cinturon = false
                            ESX.ShowNotification('~r~Cinturón quitado')
                            TriggerEvent('LIFE_CL:Sound:PlayWithinDistance', GetPlayerServerId(PlayerId()), 4, 'cinturonquitado', 0.5)
                        end
                    else
...
RegisterKeyMapping('cinturon', 'Ponerte el cinturon (vehículo)', 'keyboard', 'B')

RegisterKeyMapping('limitador', 'Poner el limitador (vehículo)', 'keyboard', '1')

RegisterNetEvent('mareofueraVehiculo')
AddEventHandler('mareofueraVehiculo', function()
...
RegisterNetEvent('mareo')
AddEventHandler('mareo', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId())
    DoScreenFadeOut(100)
